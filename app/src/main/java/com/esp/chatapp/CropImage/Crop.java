package com.esp.chatapp.CropImage;
...
public class Crop {
    ...
    public static final int REQUEST_CAMARA = 2250;
     ...
     * @param y Aspect Y
     */
    public Crop withAspect(int IsImage,int x, int y,int width,int height) {
        cropIntent.putExtra(Extra.IsImage, IsImage);
    ...
    public static void pickImage(Activity activity) {
        pickImage(activity, REQUEST_GALLERY);
    }
    public static void pickCamara(Activity activity,Uri uri) {
        pickCamara(activity, REQUEST_CAMARA,uri);
    }
    ...
    public static void pickImage(Context context, Fragment fragment) {
        pickImage(context, fragment, REQUEST_GALLERY);
    }
    ...
    public static void pickCamara(Activity activity, int requestCode,Uri uri) {
        try {
            activity.startActivityForResult(getCamaraPicker(uri), requestCode);
        } catch (ActivityNotFoundException e) {
            showImagePickerError(activity);
        }
    }
    ...
    private static Intent getCamaraPicker(Uri mImageCaptureUri) {
        Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        System.out.println("mImageCaptureUri==="+mImageCaptureUri.toString());
