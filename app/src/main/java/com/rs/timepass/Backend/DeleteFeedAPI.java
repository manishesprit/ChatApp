...
public class DeleteFeedAPI {
    ...
    public DeleteFeedAPI(Context context, ResponseListener responseListener, int feedid) {
        this.context = context;
        this.userBean = userBean;
        this.mParams = new HashMap<String, String>();
        Config.API_DELETE_FEED = Config.HOST + Config.API_DELETE_FEED_JSON + Config.userid + "=" + Pref.getValue(context, Config.PREF_USER_ID, 0) + "&" + Config.feedid + "=" + feedid;

        Log.print(":::: API_DELETE_FEED ::::" + Config.API_DELETE_FEED);
        this.responseListener = responseListener;
    }

    public void execute() {
        this.mAdapter = new Adapter(this.context);
        this.mAdapter.doGet(Config.TAG_DELETE_FEED, Config.API_DELETE_FEED, mParams,
...
//                                                false).show();
                        ...
                        responseListener.onResponce(Config.TAG_DELETE_FEED, Config.API_FAIL, context.getResources()
                                .getString(
    ...
    private void doCallBack(int code, String mesg) {
        try {
            if (code == 0) {
                responseListener.onResponce(Config.TAG_DELETE_FEED, Config.API_SUCCESS,null);
            } else if (code > 0) {
                responseListener.onResponce(Config.TAG_DELETE_FEED,
    ...
    public void doCancel() {
        if (mAdapter != null) {
            mAdapter.doCancel(Config.TAG_DELETE_FEED);
        }
    }
}