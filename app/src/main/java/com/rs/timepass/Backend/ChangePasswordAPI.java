...
public class ChangePasswordAPI {
    ...
    public ChangePasswordAPI(Context context, ResponseListener responseListener, UserBean userBean) {
        this.context = context;
        this.mParams = new HashMap<String, String>();
        Config.API_CHANGE_PASSWORD = Config.HOST + Config.API_CHANGE_PASSWORD_JSON + Config.userid + "=" + Pref.getValue(context, Config.PREF_USER_ID, 0) + "&" + Config.password + "=" + userBean.password + "&" + Config.newpassword + "=" + userBean.newpassword;
        this.userBean = userBean;

        Log.print("::::  API_CHANGE_PASSWORD::::" + Config.API_CHANGE_PASSWORD);
        this.responseListener = responseListener;
    }

    public void execute() {
        this.mAdapter = new Adapter(this.context);
        this.mAdapter.doGet(Config.TAG_CHANGE_PASSWORD, Config.API_CHANGE_PASSWORD, mParams,
...
//                                                false).show();
                        ...
                        responseListener.onResponce(Config.TAG_CHANGE_PASSWORD, Config.API_FAIL, context.getResources()
                                .getString(
                                        R.string.connectionErrorMessage));
                    }
                });
    ...
    private void doCallBack(int code, String mesg, UserBean userBean) {
        try {
            if (code == 0) {
                responseListener.onResponce(Config.TAG_CHANGE_PASSWORD,
    ...
    public void doCancel() {
        if (mAdapter != null) {
            mAdapter.doCancel(Config.TAG_CHANGE_PASSWORD);
        }
    }
}